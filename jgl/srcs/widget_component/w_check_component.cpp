...
void w_check_component::render(c_viewport *viewport)
{
	...
	fill_rectangle(_back, _anchor - viewport->anchor(), _area, viewport);
	fill_rectangle(_front, _anchor + _border - viewport->anchor(), _area - _border * 2, viewport);
	if (_state == true)
		fill_rectangle(_check, _anchor + _border * 2 - viewport->anchor(), _area - _border * 4, viewport);
}
