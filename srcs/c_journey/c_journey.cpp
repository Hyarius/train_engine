...
void c_journey::add_point(c_milestone *new_point, pair_int p_hour)
{
	...
	{
		c_hour_entry *new_hour = new c_hour_entry(p_hour.first, p_hour.second, _map);
		new_hour->set_geometry(Vector2(_map->convert_to_screen_coord(new_point->pos()) + delta), panel_size);
		new_hour->active();
...
void c_journey::actualize_panel()
{
	...
	for (size_t i = 0; i < _hour_panel.size(); i++)
	{
		if (_hour_panel[i] != nullptr)
			_hour_panel[i]->set_geometry(Vector2(_map->convert_to_screen_coord(_path[i]->pos()) + delta), panel_size);
	}
}
